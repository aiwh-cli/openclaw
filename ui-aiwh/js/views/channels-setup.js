...
// Per-channel guided setup flows. No terminal required.
  ...
  showModal(`
    <div class="ch-wizard">
      ...
      <div class="ch-instructions">
        <div class="ch-step-block">
          <p><strong>Step 1:</strong> Create a Discord Application</p>
          <ol>
            <li>Go to <a href="https://discord.com/developers/applications" target="_blank">Discord Developer Portal</a></li>
            <li>Click <strong>"New Application"</strong>, give it a name</li>
            <li>Go to <strong>Bot</strong> tab, click <strong>"Add Bot"</strong></li>
          </ol>
        </div>
        <div class="ch-step-block">
          <p><strong>Step 2:</strong> Enable Privileged Intents</p>
          <ol>
            ...
            <li>Enable <strong>Server Members Intent</strong> (recommended)</li>
          </ol>
        </div>
        <div class="ch-step-block">
          <p><strong>Step 3:</strong> Invite the Bot</p>
          <ol>
      ...
      <div class="ch-form-actions">
        <button class="btn btn-ghost" onclick="showChannelPicker()">Back</button>
        <button class="btn btn-primary" onclick="discordStep2()">Next: Enter Token</button>
      </div>
    </div>
  `);
}
...
function discordStep2() {
  showModal(`
    <div class="ch-wizard">
      ...
      <div class="ch-instructions">
        <p>In the <a href="https://discord.com/developers/applications" target="_blank">Developer Portal</a>, go to your app → <strong>Bot</strong> → click <strong>"Reset Token"</strong> and copy it.</p>
      </div>
      <div class="ch-form-group">
        <label>Bot Token</label>
        <input type="password" id="ch-discord-token" class="input" placeholder="Paste your Discord bot token">
      </div>
      <div class="ch-form-group">
        <label>Account ID <span class="ch-hint">(optional, defaults to "default")</span></label>
        <input type="text" id="ch-discord-account" class="input" placeholder="default">
      </div>
      <div class="ch-form-actions">
        <button class="btn btn-ghost" onclick="startSetup_discord()">Back</button>
        <button class="btn btn-primary" onclick="discordSubmit()">Connect Discord</button>
      </div>
    </div>
  `);
}
...
async function discordSubmit() {
  ...
  const account = document.getElementById('ch-discord-account')?.value?.trim() || undefined;
  ...
  if (r?.ok) {
    // Auto-bind to main agent (same as Telegram, Slack, WhatsApp)
    ...
    showModal(`<div class="ch-wizard">
      <div class="ch-wizard-steps"><span class="ch-step done">1. Create Bot</span><span class="ch-step done">2. Token</span><span class="ch-step active">3. Done</span></div>
      <div class="ch-wizard-success">
        <span class="ch-success-icon">&#10003;</span>
        <h3>Discord Connected</h3>
        <p>Your bot is now configured and linked to Branson. It may take a few seconds to come online.</p>
        <p class="ch-hint">You can add more agent bindings and configure server settings in Channel Settings.</p>
      </div>
...
function startSetup_telegram() {
  showModal(`
    <div class="ch-wizard">
      ...
      <div class="ch-instructions">
        <div class="ch-step-block">
          <p><strong>Step 1:</strong> Create a Bot via BotFather</p>
          <ol>
        ...
        <div class="ch-step-block">
          <p><strong>Step 2:</strong> (Optional) Enable Group Message Access</p>
          <ol>
            <li>Message @BotFather: <code>/setprivacy</code></li>
            <li>Select your bot, then choose <strong>Disable</strong></li>
            <li>This lets the bot see all group messages, not just @mentions</li>
          </ol>
        </div>
      </div>
      <div class="ch-form-actions">
        <button class="btn btn-ghost" onclick="showChannelPicker()">Back</button>
        <button class="btn btn-primary" onclick="telegramStep2()">Next: Enter Token</button>
      </div>
    </div>
  `);
}
...
function telegramStep2() {
  showModal(`
    <div class="ch-wizard">
      ...
      <h3>Enter Bot Token</h3>
      <div class="ch-instructions">
        <p>Paste the token you received from <strong>@BotFather</strong>. It looks like: <code>123456789:ABCdefGHI...</code></p>
      </div>
      <div class="ch-form-group">
        <label>Bot Token</label>
        <input type="password" id="ch-telegram-token" class="input" placeholder="123456789:ABCdefGHIjklmNOPqrs">
      </div>
      <div class="ch-form-actions">
        <button class="btn btn-ghost" onclick="startSetup_telegram()">Back</button>
        <button class="btn btn-primary" onclick="telegramSubmit()">Connect Telegram</button>
      </div>
    </div>
  `);
}
  ...
  if (r?.ok) {
    ...
    showModal(`<div class="ch-wizard">
      ...
      <div class="ch-wizard-success">
        ...
        <p>Your bot is online and bound to the <strong>main</strong> agent.</p>
        <p class="ch-hint">DM your bot on Telegram to test. By default, new users need to pair via a code.</p>
      </div>
      <div class="ch-form-actions"><button class="btn btn-primary" onclick="closeModal(); loadChannels();">Done</button></div>
    </div>`);
  } else {
...
function startSetup_slack() {
  showModal(`
    <div class="ch-wizard ch-wizard-wide">
      ...
      <div class="ch-instructions">
        <div class="ch-step-block">
          <p><strong>Step 1:</strong> Create a Slack App</p>
          <ol>
            ...
            <li>Name it and pick your workspace</li>
          </ol>
        </div>
        <div class="ch-step-block">
          <p><strong>Step 2:</strong> Enable Socket Mode</p>
          <ol>
            <li>In the sidebar, go to <strong>Socket Mode</strong> and enable it</li>
            <li>Generate an <strong>App Token</strong> (<code>xapp-...</code>) with <code>connections:write</code> scope</li>
            <li>Copy the App Token</li>
          </ol>
        </div>
        <div class="ch-step-block">
          <p><strong>Step 3:</strong> Add Bot Scopes & Install</p>
          <ol>
            <li>Go to <strong>OAuth & Permissions</strong></li>
            <li>Add these Bot Token Scopes: <code>chat:write</code>, <code>channels:history</code>, <code>channels:read</code>, <code>groups:history</code>, <code>im:history</code>, <code>im:read</code>, <code>im:write</code>, <code>app_mentions:read</code>, <code>reactions:read</code>, <code>reactions:write</code>, <code>users:read</code></li>
            <li>Go to <strong>Event Subscriptions</strong>, subscribe to: <code>app_mention</code>, <code>message.channels</code>, <code>message.groups</code>, <code>message.im</code></li>
            <li>Go to <strong>App Home</strong>, enable <strong>Messages Tab</strong></li>
            <li><strong>Install to Workspace</strong> and copy the <strong>Bot Token</strong> (<code>xoxb-...</code>)</li>
          </ol>
        </div>
      </div>
      <div class="ch-form-actions">
        <button class="btn btn-ghost" onclick="showChannelPicker()">Back</button>
        <button class="btn btn-primary" onclick="slackStep2()">Next: Enter Tokens</button>
      </div>
    </div>
  `);
}
...
function slackStep2() {
  showModal(`
    <div class="ch-wizard">
      ...
      <div class="ch-form-group">
        <label>Bot Token <span class="ch-hint">(starts with xoxb-)</span></label>
        <input type="password" id="ch-slack-bot" class="input" placeholder="xoxb-...">
      </div>
      <div class="ch-form-group">
        <label>App Token <span class="ch-hint">(starts with xapp-)</span></label>
        <input type="password" id="ch-slack-app" class="input" placeholder="xapp-...">
      </div>
      <div class="ch-form-actions">
        <button class="btn btn-ghost" onclick="startSetup_slack()">Back</button>
        <button class="btn btn-primary" onclick="slackSubmit()">Connect Slack</button>
      </div>
    </div>
  `);
}
...
async function slackSubmit() {
  const bot = document.getElementById('ch-slack-bot')?.value?.trim();
  const app = document.getElementById('ch-slack-app')?.value?.trim();
  ...
  const r = await api('/channels/add', { method: 'POST', body: {
    channel: 'slack', options: { 'bot-token': bot, 'app-token': app },
  }});
  if (r?.ok) {
    await api('/channels/bind', { method: 'POST', body: { agentId: 'main', channel: 'slack' } });
    showModal(`<div class="ch-wizard">
      <div class="ch-wizard-steps"><span class="ch-step done">1. Create App</span><span class="ch-step done">2. Tokens</span><span class="ch-step active">3. Done</span></div>
      <div class="ch-wizard-success">
        <span class="ch-success-icon">&#10003;</span>
        <h3>Slack Connected</h3>
        <p>Your Slack app is configured and bound to the <strong>main</strong> agent.</p>
        <p class="ch-hint">DM your bot in Slack or @mention it in a channel to test.</p>
      </div>
      <div class="ch-form-actions"><button class="btn btn-primary" onclick="closeModal(); loadChannels();">Done</button></div>
    </div>`);
  } else {
...
function startSetup_whatsapp() {
  showModal(`
    <div class="ch-wizard">
      ...
      <div class="ch-form-actions">
        <button class="btn btn-ghost" onclick="showChannelPicker()">Back</button>
        <button class="btn btn-primary" onclick="whatsappAddAndLink()">Continue</button>
      </div>
    </div>
  `);
}
...
async function whatsappAddAndLink() {
  ...
  // Step 2: Request QR code from gateway
  ...
  if (qr?.qrDataUrl) {
    whatsappShowQR(qr.qrDataUrl, qr.message);
  } else if (qr?.connected) {
    whatsappDone();
  } else {
    showModal(`<div class="ch-wizard">
      <div class="ch-wizard-error">Could not generate QR code. ${escHtml(qr?.error || qr?.message || 'Gateway may not be running.')}</div>
      <div class="ch-form-actions">
        <button class="btn btn-ghost" onclick="startSetup_whatsapp()">Back</button>
        <button class="btn btn-primary" onclick="whatsappAddAndLink()">Retry</button>
      </div></div>`);
  }
}

function whatsappShowQR(dataUrl, message) {
  showModal(`
    <div class="ch-wizard">
      ...
      <h3>Scan QR Code</h3>
      <div class="ch-qr-container">
        <img src="${dataUrl}" alt="WhatsApp QR Code" class="ch-qr-img">
      </div>
      <div class="ch-instructions">
        <ol>
        ...
        <p class="ch-hint">${escHtml(message || 'QR expires in 2 minutes.')}</p>
      </div>
      <div id="ch-wa-status" class="ch-wizard-loading">Waiting for scan…</div>
      <div class="ch-form-actions">
        <button class="btn btn-ghost" onclick="closeModal()">Cancel</button>
        <button class="btn btn-ghost" onclick="whatsappAddAndLink()">New QR</button>
      </div>
    </div>
  `);
...
async function whatsappWaitForScan() {
  ...
  try {
    const result = await api('/channels/whatsapp/login-wait', {
      method: 'POST', body: { timeoutMs: 120000 },
    });
    if (result?.connected) {
      await api('/channels/bind', { method: 'POST', body: { agentId: 'main', channel: 'whatsapp' } }).catch(() => {});
      // Set DM policy to allowlist (not pairing) — prevents surprise codes to unknown contacts
...
function whatsappDone() {
  showModal(`<div class="ch-wizard">
    ...
    <div class="ch-wizard-success">
      ...
      <p class="ch-hint">Only phone numbers you add to the allowlist can chat. Go to Channel Settings to add your number.</p>
    </div>
    <div class="ch-form-actions"><button class="btn btn-primary" onclick="closeModal(); loadChannels();">Done</button></div>
  </div>`);
}
