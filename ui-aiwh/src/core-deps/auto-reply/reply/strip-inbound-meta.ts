...
// Inlined: avoids zod npm dep which Vite lib mode externalizes
function safeParseJsonAsRecord(raw: string): Record<string, unknown> | null {
  try {
...
// Was: z.record(z.string(), z.unknown()) — just validates it's a plain object
...
function parseInboundMetaBlock(lines: string[], sentinel: string): Record<string, unknown> | null {
  for (let i = 0; i < lines.length; i++) {
    ...
    return safeParseJsonAsRecord(jsonText);
  }
  return null;
}
