...
import type { MessageGroup, ToolCard } from "../ui-deps/types/chat-types.ts";
...
// ── AIWH: Inline media rendering from tool results ──────────────
...
/**
 * Extract media file paths from tool card results and render as
 * inline <aiwh-media-block> elements. Detects both explicit media
 * tool results and file paths embedded in any tool output text.
 */
function renderInlineMedia(toolCards: ToolCard[]) {
  ...
  for (const card of toolCards) {
    ...
    const paths = extractMediaPaths(card.text);
    mediaPaths.push(...paths);
    // If it's a media tool but no file paths found, check for filePath JSON
    if (isMediaTool && paths.length === 0) {
      try {
        const parsed = JSON.parse(card.text);
        const fp = parsed.filePath || parsed.file_path || parsed.path || parsed.output;
        if (typeof fp === "string" && fp.length > 3) {
          const detectedPaths = extractMediaPaths(fp);
...
function renderGroupedMessage(
  ...
  if (!markdown && hasToolCards && isToolResult) {
    return html`${renderCollapsedToolCards(toolCards, onOpenSidebar)}${renderInlineMedia(toolCards)}`;
  }
