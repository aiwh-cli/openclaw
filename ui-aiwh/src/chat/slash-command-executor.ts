/**
 * Slash-command dispatcher, shared types, and shared utilities.
 * Individual command handlers live in ./slash-command-handlers.ts.
 */
...
import { normalizeThinkLevel, resolveThinkingDefaultForModel } from "../ui-deps/thinking.ts";
import type {
  ChatModelOverride,
  GatewaySessionRow,
  ModelCatalogEntry,
...
import {
  ...
  executeUsage,
  executeVerbose,
} from "./slash-command-handlers.ts";
...
export type SlashCommandResult = {
  ...
  action?:
    ...
    | "toggle-focus"
    | "navigate-usage";
  sessionPatch?: { modelOverride?: ChatModelOverride };
  /** When set, the caller should track this as the active run. */
  trackRunId?: string;
...
export function normalizeVerboseLevel(raw?: string | null): "off" | "on" | "full" | undefined {
  if (!raw) {
    return undefined;
  }
...
// ── Shared Utilities (used by handlers in slash-command-handlers.ts) ──

export function normalizeSessionKey(key?: string | null): string | undefined {
  return key?.trim().toLowerCase() || undefined;
}

export function resolveEquivalentSessionKeys(
...
export function buildSessionIndex(sessions: GatewaySessionRow[]): Map<string, GatewaySessionRow> {
...
export function isWithinCurrentSessionSubtree(
...
export async function loadThinkingCommandState(client: GatewayBrowserClient, sessionKey: string) {
...
export function resolveCurrentFastMode(session: GatewaySessionRow | undefined): "on" | "off" {
  return session?.fastMode === true ? "on" : "off";
}
export function formatDirectiveOptions(text: string, options: string): string {
  return `${text}\nOptions: ${options}.`;
}
...
export function resolveSteerSubagent(
  sessions: GatewaySessionRow[],
  currentSessionKey: string,
  target: string,
): string[] {
...
export async function resolveSteerTarget(
  client: GatewayBrowserClient,
  sessionKey: string,
  args: string,
  context: SlashCommandContext,
...
export function isActiveSteerSession(session: GatewaySessionRow | undefined): boolean {
